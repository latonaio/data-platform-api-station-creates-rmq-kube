...
import (
	...
	dpfm_api_output_formatter "data-platform-api-station-creates-rmq-kube/DPFM_API_Output_Formatter"
	"data-platform-api-station-creates-rmq-kube/config"
...
func (c *DPFMAPICaller) subfuncProcess(
	...
	for _, fn := range accepter {
		...
		case "RailwayLine":
			c.railwayLineCreate(mtx, wg, subFuncFin, input, output, subfuncSDC, errs, log)
		default:
			wg.Done()
		}
	}
...
func (c *DPFMAPICaller) platformCreate(
	mtx *sync.Mutex,
	wg *sync.WaitGroup,
	errFin chan error,
	input *dpfm_api_input_reader.SDC,
	...
	defer wg.Done()
	err = c.complementer.ComplementPlatform(input, subfuncSDC, log)
	if err != nil {
		mtx.Lock()
		*errs = append(*errs, err)
		mtx.Unlock()
...
func (c *DPFMAPICaller) railwayLineCreate(
	mtx *sync.Mutex,
	wg *sync.WaitGroup,
	errFin chan error,
	input *dpfm_api_input_reader.SDC,
	...
	defer wg.Done()
	err = c.complementer.ComplementRailwayLine(input, subfuncSDC, log)
	if err != nil {
		mtx.Lock()
		*errs = append(*errs, err)
		mtx.Unlock()
